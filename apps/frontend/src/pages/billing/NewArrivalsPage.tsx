...
function buildCartPayload(product: Product) {
  return {
    ...
    modelName: getProductDisplayName(product),
...
export default function NewArrivalsPage() {
  ...
  const { data: categories } = useQuery<Category[]>({
    ...
    queryFn: () => {
      if (selectedBrand && selectedBrand !== '_all') {
        return categoryService.getByBrand(selectedBrand);
      }
  ...
  const orderMutation = useMutation({
    mutationFn: (priceType?: 'CASH' | 'CREDIT') => orderService.create({
  ...
  return (
    <div className="pb-24 lg:pb-0">
      ...
      <PriceSelectionModal
        ...
        onConfirm={(priceType) => orderMutation.mutate(priceType)}
      />
              ...
              {items.length === 0 ? (
                ...
                <>
                  ...
                  <div className="border-t pt-3 space-y-3">
                    ...
                    <Button className="w-full h-10" onClick={handlePlaceOrder} disabled={orderMutation.isPending || !selectedStore || items.length === 0}>
